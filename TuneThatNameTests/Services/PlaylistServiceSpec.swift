        ...
        describe("The Playlist Service") {
            ...
            describe("create a playlist") {
                context("when contacts are not filtered") {
                    ...
                    context("and the contact service calls back with an error") {
                    ...
                    context("and the contact service calls back with a list of contacts with first names") {
                        ...
                        context("and the user service calls back with an error") {
                            ...
                            it("calls back with the same error") {
                                ...
                                expect(self.callbackErrorList.count).toEventually(equal(1))
                                expect(self.callbackErrorList[0]).toEventually(equal(userServiceError))
                                expect(self.callbackPlaylistList).to(beEmpty())
                            }
                        }
                        ...
                        context("and user service calls back with a user") {
                            ...
                            it("always calls the echo nest service with the expected locale") {
                                ...
                                expect(mockEchoNestService.mocker.recordedParameters[MockEchoNestService.Method.findSongs]).toEventuallyNot(beNil())
                                if let findSongsParametersList = mockEchoNestService.mocker.recordedParameters[MockEchoNestService.Method.findSongs] {
                                    ...
                                    for findSongsParameters in findSongsParametersList {
                                        expect(findSongsParameters[3] as? String).to(equal(userLocale))
                                    }
                                }
                            }
